...
def load_and_prepare_test_data(data_root_path, csv_file_path, csv_input_scaler, one_hot_cols):
    ...
    for idx in raw_test_df.index:
        ...
        test_files.append({
            ...
            "targets": raw_test_df.loc[idx, TARGET_COLUMN],
            "id": str(idx)
        })
...
def predict_and_evaluate():
    ...
    with torch.no_grad():
        for batch_data in tqdm(test_loader, desc="处理测试集"):
            inputs, targets_raw, csv_input_features = (
                batch_data["image"].to(DEVICE),
                batch_data["targets"],
                batch_data["csv_input_features"].to(DEVICE)
            )

            outputs, image_features = model_to_load(inputs, csv_input_features)

            preds = torch.argmax(outputs, dim=1).cpu().numpy()
            probs = torch.nn.functional.softmax(outputs, dim=1).cpu().numpy()

            all_preds_labels.extend(label_encoder.inverse_transform(preds))
            all_targets_labels.extend(targets_raw)
            all_preds_probs.extend(probs)

    all_preds_labels = np.array(all_preds_labels)
    all_targets_labels = np.array(all_targets_labels)
    all_preds_probs = np.array(all_preds_probs)

    # --- 4. 生成和保存测试集报告 ---
    print("\n--- 生成并保存测试集分类报告和图表 ---")

    report_path = os.path.join(REPORT_OUTPUT_DIR, "test_classification_report.txt")
    with open(report_path, "w") as f:
        f.write("--- Test Set Classification Report ---\n\n")
        f.write(classification_report(all_targets_labels, all_preds_labels, labels=class_names, zero_division=0))
    print(f"分类报告已保存至: {report_path}")

    plot_confusion_matrix(all_targets_labels, all_preds_labels, class_names, os.path.join(REPORT_OUTPUT_DIR, "test_confusion_matrix.png"))
    print("混淆矩阵图已保存。")

    plot_roc_curve_multiclass(all_targets_labels, all_preds_probs, num_classes, class_names, os.path.join(REPORT_OUTPUT_DIR, "test_roc_curve.png"))
    print("ROC曲线图已保存。")
