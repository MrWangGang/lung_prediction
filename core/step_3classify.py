...
def load_and_prepare_data_multimodal(data_root_path, csv_dir):
    ...
    try:
        ...
        for df in [train_df_raw, val_df_raw, test_df_raw]:
            ...
            if TARGET_COLUMN not in df.columns:
                raise ValueError(f"CSV 文件中缺少目标列 '{TARGET_COLUMN}'。")
        ...
        all_defined_features = ONE_HOT_INPUT_FEATURES + BINNER_INPUT_FEATURES + NORMAL_CSV_INPUT_FEATURES + [TARGET_COLUMN]
    ...
    # --- 2. 数据预处理 (标准化与独热编码) ---
    def preprocess_df(df, is_training_set, csv_input_scaler=None, label_encoder=None, one_hot_cols=None):
        preprocessed_df = df.copy()

        # 对目标列进行标签编码
        if is_training_set:
            label_encoder = LabelEncoder()
            preprocessed_df[TARGET_COLUMN] = label_encoder.fit_transform(preprocessed_df[TARGET_COLUMN])
        else:
            preprocessed_df[TARGET_COLUMN] = label_encoder.transform(preprocessed_df[TARGET_COLUMN])
        ...
        return preprocessed_df, csv_input_scaler, label_encoder, final_csv_input_features, one_hot_cols

    train_processed_df, csv_input_scaler, label_encoder, final_csv_input_features, one_hot_cols = preprocess_df(train_df, True)
    val_processed_df, _, _, _, _ = preprocess_df(val_df, False, csv_input_scaler, label_encoder, one_hot_cols)
    test_processed_df, _, _, _, _ = preprocess_df(test_df, False, csv_input_scaler, label_encoder, one_hot_cols)
    ...
    print(f"目标变量标签编码器已保存到: {label_encoder_path}")

    train_files = []
    val_files = []
    test_files = []

    def prepare_file_list(data_list_raw, processed_df, features_list, target_col):
        file_list = []
        for d in data_list_raw:
            ...
            file_list.append({
                ...
                "targets": processed_df.loc[pid, target_col],
                "id": pid
            })
        return file_list

    train_files = prepare_file_list(train_data_list_raw, train_processed_df, final_csv_input_features, TARGET_COLUMN)
    val_files = prepare_file_list(val_data_list_raw, val_processed_df, final_csv_input_features, TARGET_COLUMN)
    test_files = prepare_file_list(test_data_list_raw, test_processed_df, final_csv_input_features, TARGET_COLUMN)

    return train_files, val_files, test_files, csv_input_scaler, label_encoder, final_csv_input_features
...
def validate_and_report(data_loader, model, loss_function, set_name, label_encoder):
    model.eval()
    val_loss = 0
    step = 0
    all_preds = []
    ...
    with torch.no_grad():
        for batch_data in tqdm(data_loader, desc=f"评估 {set_name} 周期"):
            step += 1
            inputs, targets, csv_input_features = (
    ...
    # 额外返回用于最终评估的预测和真实标签
    return val_loss, report_dict, report_str, all_preds, all_targets
...
def train_model(train_loader, val_loader, test_loader, model, loss_function, optimizer,
    ...
    for epoch in range(max_epochs):
        ...
        val_loss, val_report_dict, val_report_str, val_preds, val_targets = validate_and_report(val_loader, model, loss_function, "验证集", label_encoder)
        test_loss, test_report_dict, test_report_str, test_preds, test_targets = validate_and_report(test_loader, model, loss_function, "测试集", label_encoder)
...
if __name__ == "__main__":
    ...
    train_files, val_files, test_files, csv_input_scaler, label_encoder, all_csv_features_list = \
    ...
    if os.path.exists(best_model_path):
        ...
        # 重新评估以获得准确的最终报告
        _, final_val_report_dict, final_val_report_str, _, _ = validate_and_report(val_loader, model, loss_function, "最终验证集", label_encoder)
        _, final_test_report_dict, final_test_report_str, _, _ = validate_and_report(test_loader, model, loss_function, "最终测试集", label_encoder)
