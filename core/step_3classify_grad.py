...
def main():
    ...
    if not os.path.exists(csv_input_scaler_path) or not os.path.exists(label_encoder_path):
    ...
    if image_network_name == 'ResNet':
        ...
        target_layer = list(model_to_load.image_feature_network.features.denseblock4.children())[-1].layers.conv2
    ...
    for i in tqdm(range(len(test_ds)), desc="Generating Grad-CAM for each sample"):
        ...
        preprocessed_df = pd.get_dummies(csv_df, columns=ONE_HOT_INPUT_FEATURES, prefix=ONE_HOT_INPUT_FEATURES, dummy_na=False)
        ...
        csv_input = csv_input_scaler.transform(preprocessed_df[final_csv_input_features])
